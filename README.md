...
* **Mixture builder** ("Mixture / composite" tab, and "Mix" on the Compare and Experimental pages): step 1 — add components one at a time (names, formulas, polymers, elements or custom materials; ambiguous ones such as epoxy ask you to pick a composition preset); step 2 — set the ratios with inputs or sliders in wt% or vol%, optionally letting the last component take the remainder. Nothing is pre-selected.
